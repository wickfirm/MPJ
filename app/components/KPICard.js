  ...
  return (
    <div className="kpi-card animate-count-up">
      <div className="flex items-start justify-between mb-3">
        ...
        {Icon && (
          <div className="w-8 h-8 rounded-xl bg-mpj-purple/10 flex items-center justify-center group-hover:bg-mpj-purple/20 transition-colors flex-shrink-0">
            <Icon size={15} className="text-mpj-purple" />
          </div>
        )}
      </div>
      <p className="text-2xl font-bold text-gray-900 leading-tight tabular-nums">
        {prefix}<span className="text-mpj-purple">{value}</span>{suffix}
      </p>
