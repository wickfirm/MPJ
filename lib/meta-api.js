 ...
 * Returns array of { ad_id, ad_name, campaign_id, image_hashes: [{ hash, type }] }
 */
export async function fetchAdCreatives(campaignIds, token, adAccountId) {
  const results = []

  for (const campaignId of campaignIds) {
    const params = new URLSearchParams({
      fields: 'id,name,campaign_id,created_time,creative{id,thumbnail_url,object_story_spec}',
        ...
        if (spec?.link_data) {
          ...
          if (spec.link_data.child_attachments) {
            for (const card of spec.link_data.child_attachments) {
              if (card.image_hash) hashes.push({ hash: card.image_hash, type: 'carousel' })
            }
          }
        }
